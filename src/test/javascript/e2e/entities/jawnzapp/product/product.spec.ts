...
describe('Product e2e test', () => {
  ...
  it('should create and save Products', async () => {
    ...
    await promise.all([
      productUpdatePage.setDescriptionInput('d'),
      productUpdatePage.setNameInput('aUN'),
      productUpdatePage.setImageInput(absolutePath),
    ]);

    expect(await productUpdatePage.getDescriptionInput()).to.eq('d', 'Expected Description value to be equals to d');
    expect(await productUpdatePage.getNameInput()).to.eq('aUN', 'Expected Name value to be equals to aUN');
